...
export async function createBuildByPercyToken(
  ...
  try {
    projectId = await query<{ id: number }[]>('SELECT id from projects WHERE percy_token = ? LIMIT 1', [percyToken]);
  } catch (e) {
    // todo
  ...
  if (projectId[0].id) {
    try {
      await query<unknown>('INSERT into builds (project_id, baseline_branch, branch) VALUES (?, ?, ?)', [
        projectId[0].id,
